...
(function (global, factory) {
  ...
  class Alert {
    ...
    constructor(opts) {
      this.el = opts.el;
      this.dismiss = this.el.querySelector(Selector$t.DISMISS); // Add event handlers
  ...
  */
    ...
    update(value) {
      ...
      if (typeof value === 'string') {
        ...
        if (value) {
          ...
          this.container.dispatchEvent(this[EventName$r.ON_UPDATE]); // Is empty string. Menu should be closed.
        } else {
          this.close();
        }
      }
  ...
  class Sticky {
    ...
    update(opts) {
      ...
      _classPrivateFieldLooseBase(this, _init$3)[_init$3](); // Create and dispatch custom event
  ...
  function _reallocateSlideItems() {
    ...
    if (slidesToAdd > 0) {
      ...
      for (let i = 0; i < slidesToAdd; i++) {
        ...
        const newParent = newNode.querySelector(Selector$p.ROW); // Clear out duplicated slide items
    ...
    this.slides = this.el.querySelectorAll(Selector$p.ITEM); // Reset current slide index if it's on a slide that's been removed
  ...
  class Collapse {
    ...
    constructor(_ref) {
      ...
      this[EventName$k.HIDDEN] = new CustomEvent(EventName$k.HIDDEN); // Find all auto-initialized Collapse buttons
  ...
  class CollapseControls {
    ...
    constructor(opts) {
      ...
      if (!this.collapseList.length) {
        const collapseTriggers = this.accordion.querySelectorAll(Selector$m.DATA_MOUNT); // Find Collapse instances and push Collapses with matching triggers into Collapse array
  ...
  function _initializeImageSrc() {
    ...
    const defaultElement = currentFieldSet.querySelector(Selector$j.CHECKED); // Set the default selected image
  ...
  function _initInstance(opts) {
    ...
    this.controls = [];
    const controls = this.el.querySelectorAll(Selector$j.CONTROL); // Iterate through our controls, adding an event listener to change the image
  ...
  class ComboboxSelect {
    ...
    constructor(_ref) {
      ...
      this.optionEls = el.querySelectorAll(Selector$i.ROLE_OPTION); // data
  ...
  function _onOptionChange2(index) {
    ...
    this.optionEls[index].classList.add(ClassName$d.CURRENT_ITEM); // ensure the new option is in view
  ...
  class Flyout {
    ...
    positionMenu() {
      ...
      this.menu.classList.remove(`${ClassName$c.FLYOUT}-left`, `${ClassName$c.FLYOUT}-right`, `${ClassName$c.FLYOUT}-top`, `${ClassName$c.FLYOUT}-bottom`, `${ClassName$c.FLYOUT}-align-start`, `${ClassName$c.FLYOUT}-align-end`, `${ClassName$c.FLYOUT}-align-center`); // Set the menu classes associated with position
    ...
    show() {
      ...
      this.menu.classList.add(ClassName$c.SHOW); // Store the coordinates of the associated elements for ease of reuse now that the menu has layout
    ...
    hide(opts) {
      ...
      this.menu.classList.remove(ClassName$c.SHOW); // 1. Add a class that triggers a CSS animation
  ...
  function _inputCheckReducer(input) {
    ...
    return [].slice.call(this.inputs).reduce((obj, _input) => {
      if (_input.type === type && _input.name === name) {
        ...
        const maxValid = _input.getAttribute(Selector$e.CHECKBOX_MAX); // Selector.CHECKBOX_REQUIRED attribute accepts either a boolean or integer
  ...
  class FormValidation {
    ...
    update(opts) {
      ...
      if (opts) {
        this.inputs = this.el.querySelectorAll(Selector$e.INPUTS);
        this.submit = this.el.querySelector(Selector$e.SUBMIT); // Remove event handlers
  ...
  function _setScrollbar() {
    if (this.isBodyOverflowing) {
      ...
      const stickyContent = [].slice.call(document.querySelectorAll(Selector$d.STICKY_CONTENT)); // Adjust fixed content padding
  ...
  class Modal {
    ...
    constructor(_ref) {
      ...
      this[EventName$d.CLICK_DISMISS] = new CustomEvent(EventName$d.CLICK_DISMISS); // Add event handlers
    ...
    show() {
      ...
      this.el.addEventListener(EventName$d.CLICK_DISMISS, this.hide); // Find all the dismiss attribute elements and cause the modal to hide
    ...
    remove() {
      // Remove event handlers, observers, etc.
      Util.removeEvents(this.events);
      document.removeEventListener(EventName$d.FOCUSIN, this.onDocumentFocusin); // Remove this reference from the array of instances.
    ...
    if (this.inCustomViewport()) {
      ...
      imagesLoaded(this.el, () => {
        ...
        this.collapses.forEach((c, i) => {
          ...
          const dNone = c.feature.classList.contains(ClassName$7.DISPLAY.NONE); // Set visible styles

          c.el.style.display = ClassName$7.DISPLAY.BLOCK;

          if (dNone) {
            c.feature.classList.remove(ClassName$7.DISPLAY.NONE);
          } // Calculate heights
          ...
          if (dNone) {
            c.feature.classList.add(ClassName$7.DISPLAY.NONE);
          }
  ...
  function _onShown(collapse) {
    ...
    if (this.inCustomViewport()) {
      ...
      (_this$open$feature2 = this.open.feature) == null ? void 0 : _this$open$feature2.classList.remove(ClassName$7.DISPLAY.NONE);
  ...
  function _onHide(e) {
    ...
    if (this.inCustomViewport()) {
      if (otherOpen) {
        ...
        (_this$open$feature3 = this.open.feature) == null ? void 0 : _this$open$feature3.classList.add(ClassName$7.DISPLAY.NONE);
      } else {
        e.preventDefault();
      }
    } else if (!otherOpen) {
  ...
  const ClassName$5 = { ...ClassName$c,
    POPOVER: 'popover',
    CLOSE: 'close',
    ARROW: 'arrow'
  };
  const Default$2 = { ...Default$4,
  ...
  function _valueChanged() {
    if (!this.value || this.el.value !== this.value) {
      ...
      this.el.dispatchEvent(this[EventName$8.ON_CHANGE]); // Update the stored value
  ...
  class Scrollspy {
    ...
    constructor(opts) {
      ...
      this.offset = opts.offset || parseInt(this.el.dataset[Selector$7.OFFSET], 10) || Default$1.OFFSET; // Set up private properties
  ...
  function _showHideArrow() {
    ...
    if (imageTab) {
      imagesLoaded(tabListWindow, () => {
        const arrowTargetHeight = imageTab.offsetHeight - (imageTab.querySelector(Selector$3.IMAGE_TAB_LABEL).offsetHeight || 0);
  ...
  class AutoplayCarouselControls extends CarouselControls {
    ...
    update(opts) {
      ...
      Util.removeEvents(this.events); // resets this.events in base class
